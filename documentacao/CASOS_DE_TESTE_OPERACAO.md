...
|----|------|--------------------|
| 1  | Na aba `Pacientes`, clicar no botão central "+". | O aplicativo abre a tela de cadastro `TelaCadastroPaciente`. |
...
| Id | Ação | Resultado Esperado |
|----|------|--------------------|
| 1  | Abrir o modal de detalhes do paciente que possui endereço cadastrado. | O modal exibe telefone, endereço e o ícone de rotas ao lado do endereço. |
| 2  | Clicar no ícone de rotas. | Um Bottom Sheet próprio é exibido com o título `"Como chegar"`, o endereço do paciente e as opções `"Abrir no Google Maps"` e `"Abrir no Waze"`. |
...
| 4  | Retornar ao app, clicar novamente no ícone de rotas e selecionar `"Abrir no Waze"`. | O aplicativo dispara o deep link correspondente (`https://waze.com/ul?q=Av.+Paulista,+1000+-+S%C3%A3o+Paulo`) e abre o app externo do Waze. |
...
- O Bottom Sheet de rotas não deve se sobrepor visualmente aos botões fixos do modal do paciente.
...
### CT05 - Desmarcação, Falta e Cancelamento de Sessão

#### Objetivo
Validar que o usuário consegue registrar o desfecho operacional de uma sessão agendada sem excluir o histórico do atendimento.

#### Pré-Condições
- O usuário está autenticado e no Dashboard.
- Existe uma sessão com `Situacao = "Agendado"` em `Agenda de Hoje` ou `Pendências`.

#### Passos

| Id | Ação | Resultado Esperado |
|----|------|--------------------|
| 1  | Abrir o menu de ações do card da sessão. | O menu exibe `Registrar evolução`, `Faltou com aviso`, `Faltou sem aviso`, `Cancelar pelo paciente` e `Cancelar pelo profissional`. |
| 2  | Selecionar `Cancelar pelo paciente`. | O app exibe confirmação antes de aplicar o desfecho. |
| 3  | Confirmar. | A coluna `Situacao` na aba `Agenda` é atualizada para `Cancelado pelo paciente`; o atendimento sai de `Agenda de Hoje`/`Pendências`. |
| 4  | Repetir o fluxo com outra sessão e selecionar `Faltou sem aviso`. | A coluna `Situacao` é atualizada para `Faltou sem aviso`. |

#### Resultados Esperados
- O app preserva a linha original da aba `Agenda`.
- O desfecho fica rastreável pela coluna `Situacao`.
- O Dashboard não continua mostrando sessões resolvidas como pendentes.

#### Critérios de Aceitação
- Nenhuma ação de cancelamento ou falta deve apagar dados do Google Sheets.
- Toda atualização de situação deve registrar log de auditoria.
